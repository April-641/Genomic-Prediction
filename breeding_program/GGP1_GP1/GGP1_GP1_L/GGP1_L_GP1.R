...
setwd(getwd())
GGP1_L_geno <- read.table("GGP1_L_geno.txt",header = F)
num_Pat <- 10
num_Mat <- 100
num_prog <- 10
mat_rat <- num_Mat/num_Pat
input.map <- read.csv("../../map.txt",sep = " ")
nmrk <- nrow(GGP1_L_geno)
nind <- ncol(GGP1_L_geno) / 2
...
GGP1_L_s_gambank <- list()
for (j in 1:num_Pat) {  
  a <- 2*j-1
  b <- 2*j
  Ind <- GGP1_L_geno[,a:b]
  GGP1_L_s_gambank[[j]] <- list() 
  for (i in 1:(mat_rat*num_prog)) {
    GGP1_L_s_gambank[[j]][[i]] <- genotype(geno = Ind,
                                       blk.rg = blk.rg,
...
library(dplyr)
GGP1_L_s_gam <- bind_cols(GGP1_L_s_gambank)
GGP1_L_d_gambank <- list()
for (j in (num_Pat+1):(num_Pat+num_Mat)) {  
  a <- 2*j-1
  b <- 2*j
  Ind <- GGP1_L_geno[,a:b]
  GGP1_L_d_gambank[[j]] <- list() 
  for (i in 1:num_prog) {
    GGP1_L_d_gambank[[j]][[i]] <- genotype(geno = Ind,
                                           blk.rg = blk.rg,
                                           recom.spot = recom.spot,
                                           range.hot = 4:6,  
                                           range.cold = 0, 
...
GGP1_L_d_gam <- bind_cols(GGP1_L_d_gambank)
GGP1_L_gam_geno <- cbind(GGP1_L_s_gam,GGP1_L_d_gam) 
GGP1_L_pop <- tibble(c(rep(1,num_Pat+num_Mat)))
GGP1_L_pop[,2] <- c(1:(num_Pat+num_Mat))
GGP1_L_pop[,3:4] <- GGP1_L_pop[,2]
GGP1_L_pop[,5:6] <- c(0)
GGP1_L_pop[1:num_Pat,7] <- c(1)
GGP1_L_pop[(num_Pat+1):(num_Pat+num_Mat),7] <- c(2)
colnames(GGP1_L_pop) <- c("gen","index","fam","infam","sir","dam","sex")
GGP1_L_ind_pop_pat <- GGP1_L_pop[rep(1:num_Pat,each=mat_rat*num_prog),]
GGP1_L_ind_pop_mat <- GGP1_L_pop[rep((num_Pat+1):(num_Pat+num_Mat),each=num_prog),]
GGP1_L_ind_pop <- rbind(GGP1_L_ind_pop_pat,GGP1_L_ind_pop_mat)
GGP1_L_gam_pop <- GGP1_L_ind_pop
GGP1_L_gam_pop[,2] <- c(1:nrow(GGP1_L_gam_pop))
GGP1_L_ind_gam <- tibble(GGP1_L_ind_pop[,c(1,7,2)])
for (j in 1:num_Pat) {
  tmp <- paste(GGP1_L_pop$index[j],"-",sep = "")
  for (i in 1:(mat_rat*num_prog)) {
    GGP1_L_ind_gam[((j-1)*(mat_rat*num_prog)+i),4] <- paste(tmp,i,sep = "")
  }
}
for (j in (num_Pat+1):(num_Pat+num_Mat)) {
  tmp <- paste(GGP1_L_pop$index[j],"-",sep = "")
  for (i in 1:num_prog) {
    GGP1_L_ind_gam[((j-(num_Pat+1))*num_prog+i+(mat_rat*num_prog*num_Pat)),4] <- paste(tmp,i,sep = "")
  }
}
GGP1_L_ind_gam[,5] <- GGP1_L_gam_pop[,2]
colnames(GGP1_L_ind_gam) <- c("gen","sex","ind","ind-gam","gam")
GGP1_L_S_mat <- list()
for (i in 1:num_Pat) {
  ...
  GGP1_L_S_mat[[i]] <- reproduces(pop1 = GGP1_L_gam_pop,
                                  pop1.geno.id = GGP1_L_gam_pop$index,  
                                  pop1.geno = GGP1_L_gam_geno,
                                  incols = 2, 
...
GP1_L_poplist <- list()
for(i in 1:num_Pat){
  GP1_L_poplist[[i]]<-GGP1_L_S_mat[[i]][[2]]
}
GP1_L_pop <- bind_rows(GP1_L_poplist)
GP1_L_pop[,2] <- c(1:nrow(GP1_L_pop))
GP1_L_pop[,3] <- GP1_L_pop[,2]
GP1_L_genolist <- list()
for(i in 1:num_Pat){
  GP1_L_genolist[[i]]<-GGP1_L_S_mat[[i]][[1]]
}
GP1_L_geno <- bind_cols(GP1_L_genolist)
save.image("GGP1_L_GP1.RData")
write.table(GP1_L_geno,file = "GP1_L_geno.txt",quote = F,sep = "\t",row.names = F,col.names = F)
write.table(GP1_L_pop,file = "GP1_L_pop.txt",quote = F,sep = "\t",row.names = F,col.names = T)
write.table(GGP1_L_pop,file = "GGP1_L_pop.txt",quote = F,sep = "\t",row.names = F,col.names = T)
write.table(GGP1_L_ind_gam,file = "GGP1_L_ind_gam.txt",quote = F,sep = "\t",row.names = F,col.names = T)
