...
setwd(getwd())
GGP1_Y_geno <- read.table("GGP1_Y_geno.txt",header = F)
num_Pat <- 20
num_Mat <- 500
num_prog <- 10
mat_rat <- num_Mat/num_Pat
input.map <- read.csv("../../map.txt",sep = " ")
nmrk <- nrow(GGP1_Y_geno)
nind <- ncol(GGP1_Y_geno) / 2
pos.map <- check.map(input.map = input.map, num.marker = nmrk, len.block = 1e10)blk.rg <- cal.blk(pos.map)
pos.map[,7] <- 1 
recom.spot <- as.numeric(pos.map[blk.rg[, 1], 7])
GGP1_Y_s_gambank <- list()
for (j in 1:num_Pat) {  
  a <- 2*j-1
  b <- 2*j
  Ind <- GGP1_Y_geno[,a:b]
  GGP1_Y_s_gambank[[j]] <- list() 
  for (i in 1:(mat_rat*num_prog)) {
    GGP1_Y_s_gambank[[j]][[i]] <- genotype(geno = Ind,
                                       blk.rg = blk.rg,
...
library(dplyr)
GGP1_Y_s_gam <- bind_cols(GGP1_Y_s_gambank)
GGP1_Y_d_gambank <- list()
for (j in (num_Pat+1):(num_Pat+num_Mat)) {  
  a <- 2*j-1
  b <- 2*j
  Ind <- GGP1_Y_geno[,a:b]
  GGP1_Y_d_gambank[[j]] <- list() 
  for (i in 1:num_prog) {
    GGP1_Y_d_gambank[[j]][[i]] <- genotype(geno = Ind,
                                           blk.rg = blk.rg,
                                           recom.spot = recom.spot,
                                           range.hot = 4:6, 
                                           range.cold = 0,
...
GGP1_Y_d_gam <- bind_cols(GGP1_Y_d_gambank)
GGP1_Y_gam_geno <- cbind(GGP1_Y_s_gam,GGP1_Y_d_gam)
GGP1_Y_pop <- tibble(c(rep(1,num_Pat+num_Mat)))
GGP1_Y_pop[,2] <- c(1:(num_Pat+num_Mat))
GGP1_Y_pop[,3:4] <- GGP1_Y_pop[,2]
GGP1_Y_pop[,5:6] <- c(0)
GGP1_Y_pop[1:num_Pat,7] <- c(1)
GGP1_Y_pop[(num_Pat+1):(num_Pat+num_Mat),7] <- c(2)
colnames(GGP1_Y_pop) <- c("gen","index","fam","infam","sir","dam","sex")
GGP1_Y_ind_pop_pat <- GGP1_Y_pop[rep(1:num_Pat,each=mat_rat*num_prog),]
GGP1_Y_ind_pop_mat <- GGP1_Y_pop[rep((num_Pat+1):(num_Pat+num_Mat),each=num_prog),]
GGP1_Y_ind_pop <- rbind(GGP1_Y_ind_pop_pat,GGP1_Y_ind_pop_mat)
GGP1_Y_gam_pop <- GGP1_Y_ind_pop
GGP1_Y_gam_pop[,2] <- c(1:nrow(GGP1_Y_gam_pop))
GGP1_Y_ind_gam <- tibble(GGP1_Y_ind_pop[,c(1,7,2)])
for (j in 1:num_Pat) {
  tmp <- paste(GGP1_Y_pop$index[j],"-",sep = "")
  for (i in 1:(mat_rat*num_prog)) {
    GGP1_Y_ind_gam[((j-1)*(mat_rat*num_prog)+i),4] <- paste(tmp,i,sep = "")
  }
}
for (j in (num_Pat+1):(num_Pat+num_Mat)) {
  tmp <- paste(GGP1_Y_pop$index[j],"-",sep = "")
  for (i in 1:num_prog) {
    GGP1_Y_ind_gam[((j-(num_Pat+1))*num_prog+i+(mat_rat*num_prog*num_Pat)),4] <- paste(tmp,i,sep = "")
  }
}
GGP1_Y_ind_gam[,5] <- GGP1_Y_gam_pop[,2]
colnames(GGP1_Y_ind_gam) <- c("gen","sex","ind","ind-gam","gam")
GGP1_Y_S_mat <- list()
for (i in 1:num_Pat) {
  sir_ST <- mat_rat*num_prog*i-((mat_rat*num_prog)-1)
  sir_EN <- mat_rat*num_prog*i
  dam_ST <- mat_rat*num_prog*i-((mat_rat*num_prog)-1)+num_Pat*mat_rat*num_prog
  dam_EN <- mat_rat*num_prog*i+num_Pat*mat_rat*num_prog
  GGP1_Y_S_mat[[i]] <- reproduces(pop1 = GGP1_Y_gam_pop,
                                  pop1.geno.id = GGP1_Y_gam_pop$index, 
                                  pop1.geno = GGP1_Y_gam_geno,
                                  incols = 2,
...
GP1_Y_poplist <- list()
for(i in 1:num_Pat){
  GP1_Y_poplist[[i]]<-GGP1_Y_S_mat[[i]][[2]]
}
GP1_Y_pop <- bind_rows(GP1_Y_poplist)
GP1_Y_pop[,2] <- c(1:nrow(GP1_Y_pop))
GP1_Y_pop[,3] <- GP1_Y_pop[,2]
GP1_Y_genolist <- list()
for(i in 1:num_Pat){
  GP1_Y_genolist[[i]]<-GGP1_Y_S_mat[[i]][[1]]
}
GP1_Y_geno <- bind_cols(GP1_Y_genolist)
save.image("GGP1_Y_GP1.RData")
write.table(GP1_Y_geno,file = "GP1_Y_geno.txt",quote = F,sep = "\t",row.names = F,col.names = F)
write.table(GP1_Y_pop,file = "GP1_Y_pop.txt",quote = F,sep = "\t",row.names = F,col.names = T)
write.table(GGP1_Y_pop,file = "GGP1_Y_pop.txt",quote = F,sep = "\t",row.names = F,col.names = T)
write.table(GGP1_Y_ind_gam,file = "GGP1_Y_ind_gam.txt",quote = F,sep = "\t",row.names = F,col.names = T)
