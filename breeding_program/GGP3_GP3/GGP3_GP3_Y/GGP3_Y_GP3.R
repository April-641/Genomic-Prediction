...
setwd(getwd())
GGP3_Y_geno <- read.table("../../GGP3_produce/GGP3_Y_produce/GGP3_Y_geno.txt",header = F)
...
nmrk <- nrow(GGP3_Y_geno)
nind <- ncol(GGP3_Y_geno) / 2
...
GGP3_Y_s_gambank <- list()
for (j in 1:num_Pat) {  
  a <- 2*j-1
  b <- 2*j
  Ind <- GGP3_Y_geno[,a:b]
  GGP3_Y_s_gambank[[j]] <- list() 
  for (i in 1:(mat_rat*num_prog)) {
    GGP3_Y_s_gambank[[j]][[i]] <- genotype(geno = Ind,
                                       blk.rg = blk.rg,
...
library(dplyr)
GGP3_Y_s_gam <- bind_cols(GGP3_Y_s_gambank)
GGP3_Y_d_gambank <- list()
for (j in (num_Pat+1):(num_Pat+num_Mat)) {  
  a <- 2*j-1
  b <- 2*j
  Ind <- GGP3_Y_geno[,a:b]
  GGP3_Y_d_gambank[[j]] <- list() 
  for (i in 1:num_prog) {
    GGP3_Y_d_gambank[[j]][[i]] <- genotype(geno = Ind,
                                           blk.rg = blk.rg,
                                           recom.spot = recom.spot,
                                           range.hot = 4:6,  
                                           range.cold = 0, 
...
GGP3_Y_d_gam <- bind_cols(GGP3_Y_d_gambank)
GGP3_Y_gam_geno <- cbind(GGP3_Y_s_gam,GGP3_Y_d_gam) 
GGP3_Y_ind_pop_pat <- GGP3_Y_pop[rep(1:num_Pat,each=mat_rat*num_prog),]
GGP3_Y_ind_pop_mat <- GGP3_Y_pop[rep((num_Pat+1):(num_Pat+num_Mat),each=num_prog),]
GGP3_Y_ind_pop <- rbind(GGP3_Y_ind_pop_pat,GGP3_Y_ind_pop_mat)
GGP3_Y_gam_pop <- GGP3_Y_ind_pop
GGP3_Y_gam_pop[,2] <- c(1:nrow(GGP3_Y_gam_pop))  
GGP3_Y_ind_gam <- tibble(GGP3_Y_ind_pop[,c(1,7,2)])
for (j in 1:num_Pat) {
  tmp <- paste(GGP3_Y_pop$index[j],"-",sep = "")
  for (i in 1:(mat_rat*num_prog)) {
    GGP3_Y_ind_gam[((j-1)*(mat_rat*num_prog)+i),4] <- paste(tmp,i,sep = "")
  }
}
for (j in (num_Pat+1):(num_Pat+num_Mat)) {
  tmp <- paste(GGP3_Y_pop$index[j],"-",sep = "")
  for (i in 1:num_prog) {
    GGP3_Y_ind_gam[((j-(num_Pat+1))*num_prog+i+(mat_rat*num_prog*num_Pat)),4] <- paste(tmp,i,sep = "")
  }
}
GGP3_Y_ind_gam[,5] <- GGP3_Y_gam_pop[,2]
colnames(GGP3_Y_ind_gam) <- c("gen","sex","ind","ind-gam","gam")
GGP3_Y_S_mat <- list()
for (i in 1:num_Pat) {
  ...
  GGP3_Y_S_mat[[i]] <- reproduces(pop1 = GGP3_Y_gam_pop,
                                  pop1.geno.id = GGP3_Y_gam_pop$index,  
                                  pop1.geno = GGP3_Y_gam_geno,
                                  incols = 2, 
...
GP3_Y_poplist <- list()
for(i in 1:num_Pat){
  GP3_Y_poplist[[i]]<-GGP3_Y_S_mat[[i]][[2]]
}
GP3_Y_pop <- bind_rows(GP3_Y_poplist)
GP3_Y_pop[,2] <- c(1:nrow(GP3_Y_pop))
GP3_Y_pop[,3] <- GP3_Y_pop[,2]
GP3_Y_genolist <- list()
for(i in 1:num_Pat){
  GP3_Y_genolist[[i]]<-GGP3_Y_S_mat[[i]][[1]]
}
GP3_Y_geno <- bind_cols(GP3_Y_genolist)
save.image("GGP3_Y_GP3.RData")
write.table(GP3_Y_geno,file = "GP3_Y_geno.txt",quote = F,sep = "\t",row.names = F,col.names = F)
write.table(GP3_Y_pop,file = "GP3_Y_pop.txt",quote = F,sep = "\t",row.names = F,col.names = T)
write.table(GGP3_Y_ind_gam,file = "GGP3_Y_ind_gam.txt",quote = F,sep = "\t",row.names = F,col.names = T)
