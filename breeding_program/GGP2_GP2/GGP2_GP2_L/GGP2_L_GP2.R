  ...
  function(pop1 = NULL,
    ...
    if (!is.null(pop1.geno.id) & !is.null(pop1.geno)) {
      if (length(pop1.geno.id)*incols != ncol(pop1.geno)) 
        stop("Genotype IL should match genotype matrix in the first population!")
    }
    if (!is.null(pop2.geno.id) & !is.null(pop2.geno)) {
      if (length(pop2.geno.id)*incols != ncol(pop2.geno)) 
        stop("Genotype IL should match genotype matrix in the second population!")
    }
    if (mtd.reprod == "clone") {
...
nmrk <- nrow(GGP2_L_geno)
nind <- ncol(GGP2_L_geno) / 2
...
GGP2_L_s_gambank <- list()
for (j in 1:num_Pat) {  
  a <- 2*j-1
  b <- 2*j
  Ind <- GGP2_L_geno[,a:b]
  GGP2_L_s_gambank[[j]] <- list() 
  for (i in 1:(mat_rat*num_prog)) {
    GGP2_L_s_gambank[[j]][[i]] <- genotype(geno = Ind,
                                       blk.rg = blk.rg,
...
library(dplyr)
GGP2_L_s_gam <- bind_cols(GGP2_L_s_gambank)
GGP2_L_d_gambank <- list()
for (j in (num_Pat+1):(num_Pat+num_Mat)) {  
  a <- 2*j-1
  b <- 2*j
  Ind <- GGP2_L_geno[,a:b]
  GGP2_L_d_gambank[[j]] <- list() 
  for (i in 1:num_prog) {
    GGP2_L_d_gambank[[j]][[i]] <- genotype(geno = Ind,
                                           blk.rg = blk.rg,
                                           recom.spot = recom.spot,
                                           range.hot = 4:6,  
                                           range.cold = 0, 
...
GGP2_L_d_gam <- bind_cols(GGP2_L_d_gambank)
GGP2_L_gam_geno <- cbind(GGP2_L_s_gam,GGP2_L_d_gam)
GGP2_L_ind_pop_pat <- GGP2_L_pop[rep(1:num_Pat,each=mat_rat*num_prog),]
GGP2_L_ind_pop_mat <- GGP2_L_pop[rep((num_Pat+1):(num_Pat+num_Mat),each=num_prog),]
GGP2_L_ind_pop <- rbind(GGP2_L_ind_pop_pat,GGP2_L_ind_pop_mat)
GGP2_L_gam_pop <- GGP2_L_ind_pop
GGP2_L_gam_pop[,2] <- c(1:nrow(GGP2_L_gam_pop))   
GGP2_L_ind_gam <- tibble(GGP2_L_ind_pop[,c(1,7,2)])
for (j in 1:num_Pat) {
  tmp <- paste(GGP2_L_pop$index[j],"-",sep = "")
  for (i in 1:(mat_rat*num_prog)) {
    GGP2_L_ind_gam[((j-1)*(mat_rat*num_prog)+i),4] <- paste(tmp,i,sep = "")
  }
}
for (j in (num_Pat+1):(num_Pat+num_Mat)) {
  tmp <- paste(GGP2_L_pop$index[j],"-",sep = "")
  for (i in 1:num_prog) {
    GGP2_L_ind_gam[((j-(num_Pat+1))*num_prog+i+(mat_rat*num_prog*num_Pat)),4] <- paste(tmp,i,sep = "")
  }
}
GGP2_L_ind_gam[,5] <- GGP2_L_gam_pop[,2]
colnames(GGP2_L_ind_gam) <- c("gen","sex","ind","ind-gam","gam")
GGP2_L_S_mat <- list()
for (i in 1:num_Pat) {
  ...
  GGP2_L_S_mat[[i]] <- reproduces(pop1 = GGP2_L_gam_pop,
                                  pop1.geno.id = GGP2_L_gam_pop$index,  
                                  pop1.geno = GGP2_L_gam_geno,
                                  incols = 2, 
...
GP2_L_poplist <- list()
for(i in 1:num_Pat){
  GP2_L_poplist[[i]]<-GGP2_L_S_mat[[i]][[2]]
}
GP2_L_pop <- bind_rows(GP2_L_poplist)
GP2_L_pop[,2] <- c(1:nrow(GP2_L_pop))
GP2_L_pop[,3] <- GP2_L_pop[,2]
GP2_L_genolist <- list()
for(i in 1:num_Pat){
  GP2_L_genolist[[i]]<-GGP2_L_S_mat[[i]][[1]]
}
GP2_L_geno <- bind_cols(GP2_L_genolist)
save.image("GGP2_L_GP2.RData")
write.table(GP2_L_geno,file = "GP2_L_geno.txt",quote = F,sep = "\t",row.names = F,col.names = F)
write.table(GP2_L_pop,file = "GP2_L_pop.txt",quote = F,sep = "\t",row.names = F,col.names = T)
write.table(GGP2_L_ind_gam,file = "GGP2_L_ind_gam.txt",quote = F,sep = "\t",row.names = F,col.names = T)
