...
GP_L_ind <- 1000
genetic_variance <- 1
Genetic_effect <- read.table("GP1_L_TBV.txt",header=F)
...
save.image("GP1_L_Phe.RData")
write.table(phe01,"GP1_L_Phe01.txt",quote = F,sep = "\t",row.names = F,col.names = F)
write.table(phe03,"GP1_L_Phe03.txt",quote = F,sep = "\t",row.names = F,col.names = F)
write.table(phe05,"GP1_L_Phe05.txt",quote = F,sep = "\t",row.names = F,col.names = F)
