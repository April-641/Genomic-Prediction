...
GGP1_Y_vcf <- read.table("./GGP1_Y_ref.vcf",header = T)
qtn_row <- read.table("../../qtn_row.txt",header=F)
qtn_row <- as.numeric(qtn_row$V1)
qtn_geno <- GGP1_Y_vcf[c(qtn_row),]
...
save.image("GGP1_Y_TBV.RData")
write.table(qtn_geno,"GGP1_Y_qtn_geno.txt",quote = F,sep = "\t",row.names = F,col.names = T)
write.table(Genetic_effect,"GGP1_Y_TBV.txt",quote = F,sep = "\t",row.names = F,col.names = F)
